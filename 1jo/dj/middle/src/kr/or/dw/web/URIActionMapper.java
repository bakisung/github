...
//uri_mapping.properties 파일에 설정된 uri에 맞는 Action객체를 가져와서 
//인스턴스를 생성해서 반환하는 역할을 수행하는 클래스
public class URIActionMapper {
	
	//properties의 내용을 읽어와 uri는 키값, action 객체명은 value값으로 해서 저장될 Map객체 생성
	...
	//인수값으로 주어진 uri에 맞은 Action 객체를 인스턴스화해서 반환하는 메서드
	public static IAction getAction(String uri) {
		...
		if(actionMap.containsKey(uri)) {
			try {
				...
				//메모리에 로딩된 Action클래스를 '인스턴스화'한다. ==> 객체를 생성한다.
