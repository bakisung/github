			...
			<div class="col-md-10" style="max-width: 1100px;">
				<div class="card card-outline card-info">
					<div class="card-header" style="border-bottom: none;">
						...
						<form id="searchForm2" method="post" action="<%= request.getContextPath() %>/management/extrapay.do?mcode=${mcode }&murl=${murl }" style="display: contents;">
							<select class="form-control col-md-2" name="searchType" id="searchType" style="font-size: 0.8em;">
								<option value="all" ${cri.searchType eq 'all' ? 'selected' : '' }>전  체</option>
								<option value="d" ${cri.searchType eq 'a' ? 'selected' : '' }>수당코드</option>
								<option value="t" ${cri.searchType eq 'b' ? 'selected' : '' }>수당명</option>
							</select>
